...
async function postNewWork() {
  ...
  console.log("Valeurs récupérées du DOM:");
  ...
  // Vérifie si l'URL de l'image est un blob
  ...
  try {
    ...
    // Ajout dynamique du projet dans la galerie principale
    const galleryDiv = document.querySelector(".gallery");
    const newFigure = createGalleryProject(result);
    galleryDiv.appendChild(newFigure);

    // Ajout dynamique du projet dans la galerie de suppression
    const deleteGallery = document.getElementById("delete-gallery");
    const newDeleteElement = createDeleteGalleryProject(result);
    deleteGallery.appendChild(newDeleteElement);

    // Masquer la modale "Ajout photo" et afficher la modale "Galerie de suppression"
    hideModalAjout(); // Masque la modale `#modal-ajout`
    displayModalDelete(); // Affiche la modale `#modal-delete`

  } catch (error) {
    console.error("Erreur lors du post du nouveau projet : ", error);
  }

  console.log("Fin de la fonction postNewWork");
}

// Fonction pour créer un élément de galerie pour la galerie principale
function createGalleryProject(project) {
  const figure = document.createElement("figure");
...
// Fonction pour créer un élément pour la galerie de suppression
function createDeleteGalleryProject(project) {
  const elementFusionne = document.createElement("div");
  elementFusionne.id = "element-fusionne";
  elementFusionne.dataset.id = project.id;
  ...
  fondIcone.addEventListener("click", async (event) => {
    event.preventDefault();
    await deleteWork(project.id, elementFusionne);
  });

  return elementFusionne;
}

// Fonction pour masquer la modale "Ajout photo"
function hideModalAjout() {
  const modalAjout = document.getElementById("modal-ajout");
  if (modalAjout) {
    modalAjout.style.display = "none"; // Masque la modale `#modal-ajout`
    resetAjoutModal(); // Réinitialise les champs de la modale `#modal-ajout`
  } else {
    console.error("La modale 'modal-ajout' n'a pas été trouvée.");
  }
}

// Fonction pour réinitialiser les champs de la modale "Ajout photo"
function resetAjoutModal() {
  // Réinitialiser l'image sélectionnée
  const fenetreAjoutPhoto = document.getElementById("fenetre-ajout-photo");
  if (fenetreAjoutPhoto) {
    fenetreAjoutPhoto.innerHTML = `
      <i class="fa-regular fa-image" style="color: #b9c5cc; font-size: 68px; margin-top: 20px;"></i>
      <input id="bouton-plus-ajouter-photo" type="button" value="+ Ajouter photo" />
      <p>jpg, png: 4 mo max</p>
    `;
  }

  // Réinitialiser les champs texte et sélection
  document.querySelector('input[name="crea-titre-projet"]').value = "";
  document.querySelector('select[name="categorie"]').value = "";

  // Réattacher l'événement pour le bouton "Ajouter photo"
  document.getElementById("bouton-plus-ajouter-photo").addEventListener("click", function (event) {
    event.preventDefault();
    handlePhotoUpload(); // Fonction pour gérer l'upload de la photo
  });
}

// Fonction pour afficher la modale "Galerie de suppression"
function displayModalDelete() {
  const modalDelete = document.getElementById("modal-delete");
  if (modalDelete) {
    modalDelete.style.display = "flex"; // Affiche la modale `#modal-delete`
  } else {
    console.error("La modale 'modal-delete' n'a pas été trouvée.");
  }
}
...
document.getElementById("bouton-valider-crea").addEventListener("click", async function (event) {
  event.preventDefault(); 
  console.log("Bouton 'valider' cliqué, appel de la fonction postNewWork...");
  await postNewWork(); 
});
